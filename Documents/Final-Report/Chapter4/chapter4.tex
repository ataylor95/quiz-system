...
The last major problem was that it does not seem to support the WebSocket functionality of the site. This is a major component of how quizzes are run, meaning that a lot of tests do not actually test these fully and instead have to use things like browser refreshes. This is not representative of what the user would do and so somewhat defeats the point of browser tests. The user tests somewhat make up for this shortfall however. There does not appear to be a reason or explanation for this, however it could potentially be Dusk blocking the sending and recieving messages to an external sevice as the testing environment .
